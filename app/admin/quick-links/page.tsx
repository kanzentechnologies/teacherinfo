...
import { getPostsByCategory, deletePost, Post } from '@/lib/postStore';

export default function UsefulLinksManagementPage() {
  const [posts, setPosts] = useState<Post[]>([]);

  useEffect(() => {
    const fetchData = async () => {
      // Fetch only posts belonging to the useful-links category
      const fetchedPosts = await getPostsByCategory('useful-links');
      ...
      <div className="flex flex-col gap-6">
        <div className="bg-white border border-border-main p-4 sm:p-6 flex justify-between items-center">
          <div>
            ...
            <p className="text-sm text-text-muted">Manage the content and resources inside /category/useful-links/</p>
          </div>
          <Link 
            href="/admin/posts/new?category=useful-links" 
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-left">
              ...
              <tbody className="divide-y divide-border-main">
                {posts.map((post) => (
                  <tr key={post.id} className="hover:bg-hover-bg">
                    ...
                    <td className="px-4 py-3 text-right">
                      <Link href={`/admin/posts/new?edit=${post.id}&category=useful-links`} className="text-secondary hover:underline mr-3">Edit</Link>
                ...
                {posts.length === 0 && (
                  <tr>
                    <td colSpan={5} className="text-center py-6 text-text-muted italic">No records found for Useful Links category.</td>
                  </tr>
                )}
              </tbody>
            </table>
